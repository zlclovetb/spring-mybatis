package com.paradm.mybatis.tools;
...
public class XmlElementGeneratorTools {
  ...
  public static void useGeneratedKeys(XmlElement element, IntrospectedTable introspectedTable, String prefix) {
    ...
    if (gk != null) {
      IntrospectedColumn introspectedColumn = JavaElementGeneratorTools.safeGetColumn(introspectedTable, gk.getColumn());
  ...
  private static void generateSelectiveCommColumnTo(XmlElement element, IntrospectedColumn introspectedColumn, String prefix, int type) {
    switch (type) {
      case 3:
          element.addElement(
              new TextElement(MyBatis3FormattingUtilities.getEscapedColumnName(introspectedColumn) + " = " + MyBatis3FormattingUtilities.getParameterClause(introspectedColumn, prefix) + ","));
        break;
      case 2:
